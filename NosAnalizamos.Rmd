...
tabla <- datos %>%
   summarise(across(c(ALG, ANM, FP, DCS, MD),
                    list(
                      ...
                      valor_medio = ~mean(., na.rm=T), #el valor medio es la media
                      desviacion_tipica = ~sd(., na.rm=T), #existe la función sd, no hace falta hacer sqrt(var())
                      percentil75 = ~quantile(., 0.75, na.rm=T),
                      valor_maximo = ~max(., na.rm=T))))
